...
fn build_sequence(page: usize, page_size: usize, total: usize) -> Result<Sequence> {
    let seq = if page_size == 0 {
        ...
        let page_size = page_size.min(total);
        let from_val = cursor as u32;
        while cursor > 1 && count < page_size {
            count += 1;
            cursor -= 1;
        }
        let to_val = cursor as u32;

        // Ensure smaller value comes first (正序: small:large)
        let (min, max) = if from_val < to_val {
            (from_val, to_val)
        } else {
            (to_val, from_val)
        };

        let from = SeqOrUid::Value(NonZeroU32::new(min).unwrap());
        let to = SeqOrUid::Value(NonZeroU32::new(max).unwrap());
